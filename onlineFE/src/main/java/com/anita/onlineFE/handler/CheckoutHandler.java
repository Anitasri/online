...
import com.anita.onlineBE.model.dto.OrderItem;
import com.anita.onlineBE.model.dto.User;
import com.anita.onlineFE.model.CheckoutModel;
...
public class CheckoutHandler {
	...
	public CheckoutModel init(String name) throws Exception{
		...
		if(user!=null) {
			...
			if(cartLines.size() == 0 ) {
				throw new Exception("There are no items available for checkout now!");
			}
	...
	public String saveOrder(CheckoutModel checkoutModel) {
		...
		for(CartLine cartLine : cartLines) {
			...
			orderItem.setItem(cartLine.getItem());
			orderItem.setItemCount(cartLine.getItemCount());
			...
			item.setQuantity(item.getQuantity() - cartLine.getItemCount());
			item.setPurchases(item.getPurchases() + cartLine.getItemCount());
			itemDAO.update(item);
